...
worry_level_common_denominator = monkeys.reduce(1) { |total, monkey| total * monkey[2] }

def calculate_worry_level(item, operation, stress_relief)
  op = operation[0]
...
10000.times do |j|
  monkeys.count.times do |i|
    ...
    items&.each do |item|
      worry_level = calculate_worry_level(item, operation, worry_level_common_denominator)
